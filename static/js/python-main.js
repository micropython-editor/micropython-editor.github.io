...
This editor uses the TouchDevelop API for loading and saving work. It will
...
(function (TDev) {
    ...
    var outer = null;  // used to reference the window within which this editor is embedded.
    var origin = null;  // the origin domain for the outer host.
    ...
    // Posts a message back to the origin (the outer window).
    function post(message) {
        if (!outer)
            console.error("Invalid state");
        console.log('Posting to host application: ', message);
    ...
    // This function is called by the hosting window to cause the editor to be initialised. It sets things up so the user sees their code or, in the case of a new program, uses some sane defaults.
    ...
    // Save the code back to hosting window.
